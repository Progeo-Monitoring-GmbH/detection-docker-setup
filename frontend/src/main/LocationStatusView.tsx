...
const SEVERITY_ORDER: Record<Severity, number> = { beobachten: 0, alarm: 1, kritisch: 2 };

const LocationStatusView = ({ location, locationId }: LocationStatusViewProps) => {
  const auth = useAuth();
  ...
  const loadClusters = useCallback(() => {
    ...
    void axiosConfig.perform_get(
      ...
      (response) => {
        setClusters((response?.data?.clusters || []) as VerdachtsstelleCluster[]);
      ...
      (error) => {
        if (error?.response?.status === 403) {
          ...
          const reason = error?.response?.data?.reason || error.message;
          showErrorBar(enqueueSnackbar, `Could not load suspected leaks: ${reason}`);
        }
        setClustersLoading(false);
      },
    );
  ...
  const highestReading = useMemo(() => {
    return clusters.reduce<number | null>((best, cluster) => {
      ...
      return best == null || cluster.max_value > best ? cluster.max_value : best;
    }, null);
  }, [clusters]);
  ...
  const kpiTiles = clustersDenied
    ...
    : [
        ...
        },
        {
          ...
          value: highestReading != null ? String(Math.round(highestReading)) : '–',
          unit: highestReading != null ? 'mV' : undefined,
        },
        {
          label: t('status_kpi_anlage'),
          value: isOnline ? t('status_kpi_anlage_online') : t('status_kpi_anlage_offline'),
        ...
        (alarmId) =>
          new Promise<void>((resolve) => {
            void axiosConfig.perform_post(
              ...
              (error) => {
                const reason = error?.response?.data?.reason || error.message;
                showErrorBar(enqueueSnackbar, `Could not acknowledge alarm: ${reason}`);
                resolve();
              },
            );
          }),
              ...
              <>
                ...
                <div style={{ marginTop: 14 }}>
                  <LegendGradientBar
                    ...
                    ticks={
                      thresholdValue
                        ? ['0 mV', `${thresholdValue} mV`, `${thresholdValue * 3} mV`]
                        : ['0 mV']
                    }
                  />
                </div>
          ...
          <PanelCard
            ...
            actions={
              <a
                ...
                style={{ fontSize: 12.5, color: 'var(--progeo-orange)', fontWeight: 500 }}
              >
                {t('status_view_all')}
              </a>
            }
      ...
      >
        ...
        <span style={{ flex: 1, minWidth: 0 }}>
          <span style={{ display: 'flex', alignItems: 'baseline', gap: 9, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 14, fontWeight: 500 }}>{cluster.device_label}</span>
            <StateBadge state={cluster.state} label={t(`ui_${cluster.state}`)} />
            <SeverityBadge
              severity={cluster.severity}
              label={t(`ui_severity_${cluster.severity}`)}
            />
          </span>
          <span style={{ display: 'block', marginTop: 4, fontSize: 12.5, color: '#8B8383' }}>
            {cluster.since &&
              t('status_since', { date: new Date(cluster.since).toLocaleDateString() })}
          </span>
        </span>
        <span style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--progeo-orange)' }}>
          {cluster.max_value != null ? `${Math.round(cluster.max_value)} mV` : '–'}
        </span>
      </button>

      {expanded && (
        <div style={{ padding: '0 14px 14px 36px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div
            style={{
              fontSize: 10.5,
              letterSpacing: '.09em',
          ...
          <div style={{ background: 'var(--progeo-surface)', borderRadius: 11, overflow: 'hidden' }}>
            ...
            {cluster.sensors.map((sensor, index) => (
              ...
              >
                <span style={{ fontWeight: 600 }}>#{sensor.sensor_id ?? '–'}</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <button
              ...
              style={{
                ...
                border: 'none',
                borderRadius: 10,
                background: hasPendingAck ? 'var(--progeo-orange)' : 'var(--progeo-track-soft)',
            ...
            >
              {acknowledging ? <Spinner size="sm" animation="border" /> : <CheckLg size={14} />}
            ...
            {cluster.ack_by && (
              <span style={{ fontSize: 11.5, color: '#8B8383' }}>
                {t('status_acknowledged_by', {
                  name: cluster.ack_by,
                  time: cluster.ack_at ? new Date(cluster.ack_at).toLocaleString() : '–',
                })}
              </span>
            )}
          </div>
